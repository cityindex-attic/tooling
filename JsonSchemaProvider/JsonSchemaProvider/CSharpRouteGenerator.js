...
(function () {
    ...
    var generator = window.CSharpRouteGenerator = function (smd, schema, namespaceName, className, imports, routesPatch) {
        this.smd = smd;
        this.schema = schema;
        this._lines = [];
    ...
    generator.prototype = {
        ...
        generate: function () {
            ...
            each(services, function (service, key) {
                ...
                if (this.routesPatch) {
                    if (this.routesPatch[key]) {

                        if (this.routesPatch[key].visibility) {
                            visibility = this.routesPatch[key].visibility;
                        }
                        if (this.routesPatch[key]["class"]) {
                            subClassName = this.routesPatch[key]["class"];
                        }
                    }
                }
            ...
            self.writeLine("            : base(uri, new RequestController(TimeSpan.FromSeconds(0), 2, new RequestFactory(), new ErrorResponseDTOJsonExceptionFactory(), new ThrottedRequestQueue(TimeSpan.FromSeconds(5), 30, 10, \"data\"), new ThrottedRequestQueue(TimeSpan.FromSeconds(3), 1, 3, \"trading\")))");
            self.writeLine("        {");
